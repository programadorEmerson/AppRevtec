...
class DetalhesDoProdutoSemBotao extends StatefulWidget {
  String idProduto;
  DetalhesDoProdutoSemBotao(this.idProduto);

  @override
  _DetalhesDoProdutoSemBotaoState createState() =>
      _DetalhesDoProdutoSemBotaoState();
}

class _DetalhesDoProdutoSemBotaoState extends State<DetalhesDoProdutoSemBotao> {
  ...
  _populateStringArray(String idProduto, String destaque) async {
    ...
    setState(() {
      try {
        ...
        Util.linkDoPdf =
            "http://revtec.com.br/boletim/" + dados["urlBoletim"] + ".pdf";
        urlBoletim =
            "http://revtec.com.br/boletim/" + dados["urlBoletim"] + ".pdf";
        _criarPdf();
        _r = 10;
        ...
        children: <Widget>[
          ...
          Positioned(
            ...
            child: FloatingActionButton(
              onPressed: () {
                if (_selectedLocation == "Selecione a Litragem") {
                  Util.notification(
                      context, // posição
                      "Ops!!!", // Titulo
                      "Selecione a litragem desejada", // Texto
                  ...
                  if (Util.tipoDoUsuarioLogado == "Usuário" ||
                      Util.tipoDoUsuarioLogado == "Visitante" ||
                      Textos.usuarioEstaLogado == false) {
                    Util.notification(
                        context, // posição
                        "Atenção!!!", // Titulo
                        "Para pedidos faça o cadastre-se e solicite a liberação para a empresa.", // Texto
                        Icon(Icons.info_outline,
                            size: 25, color: Colors.green.shade300), // Icone
                        Colors.black, // Cor de Fundo
                        Colors.black, // Cor da barra lateral
                        Duration(seconds: 5) // duraçao em segundos
                        );
                  } else {
                    _quantidade();
                  }
      ...
      if (_litragens[i] == litragem) {
        if (Util.tipoDoUsuarioLogado == "Usuário" ||
          ...
          Util.notification(
              context, // posição
              "Atenção!!!", // Titulo
              "Para visualizar os preços faça o login. e solicite a liberação do cadastro para a empresa.", // Texto
              Icon(Icons.info_outline,
                  size: 25, color: Colors.green.shade300), // Icone
              Colors.black, // Cor de Fundo
              Colors.black, // Cor da barra lateral
          ...
          if (Textos.usuarioEstaLogado) {
            setState(() {
              ...
              if (Util.tipoDoUsuarioLogado == "Usuário" &&
                  widget.idProduto == "p146") {
                if (_selectedLocation == "500 Ml") {
                  _valorUnitario = Util.formatarValoresParaExibicao(
                      double.parse(Util.precoAlcool500ml));
                } else if (_selectedLocation == "1 Litro") {
                  _valorUnitario = Util.formatarValoresParaExibicao(
                      double.parse(Util.precoAlcool1litro));
                } else {
                  _valorUnitario = Util.formatarValoresParaExibicao(
                      double.parse(Util.precoAlcool5litros));
                }
              } else {
                _valorUnitario = Util.formatarValoresParaExibicao(
                    (double.parse(_valores[i - 1]) * (Util.porcentagem / 100) +
                        double.parse(_valores[i - 1])));
              }
            });
          } else {
            Util.notification(
